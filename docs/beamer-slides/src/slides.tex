...
\begin{frame}{Quantum Fourier Transform}
    ...
    \end{equation*}
    leading to $\text{QFT}(\ket{x})= \sum_j x_j QFT(\ket{j})$  expressed in the fourier basis. Unitary matrix representation:
    \begin{equation*}
...
\begin{frame}{QPE: Inverse Quantum Fourier Transform}
    ...
    \end{equation*}
    Thus, $\text{IQFT}(\text{QFT}(\ket{N_c\phi})) = \ket{N_c\phi}$
...
\begin{frame}{Ancilla qubit}
    ...
    \begin{equation*}
        \ket{\Psi_6} = \frac{1}{\sum_{j=0}^{2^{n_b}-1} |\frac{\beta_j N_c}{\tilde{\lambda}_j}|^2}\sum_{j=0}^{2^{n_b}-1} \frac{C}{\tilde{\lambda}_j}\beta_j\ket{u_j}\ket{\tilde{\lambda_j}}\ket{1}_a
    \end{equation*}
\end{frame}
